...
class Evaluator:
    def __init__(
        self,
        concept_annotation_dir: str,
        concept_prediction_dir: str,
        assertion_annotation_dir: str,
        assertion_prediction_dir: str,
        relation_annotation_dir: str,
        relation_prediction_dir: str,
        entries_dir: str,
    ):
    ...
    def evaluate(self):
        ...
        print("\n\n############# GLOBAL SCORE #############")
        print(f"F1 CONCEPTS", f1_concept)
        print(f"F1 ASSERTIONS", f1_assertion)
        print(f"F1 RELATIONS", f1_rel)
    ...
    def evaluate_relation(self) -> float:
        ...
        for filename in filenames:
            ground_truth_relations = self._load_relation_annotation_file(
                os.path.join(self.relation_annotation_dir, filename)
            )
            prediction_relations = self._load_relation_annotation_file(
                os.path.join(self.relation_prediction_dir, filename)
            )
        ...
        for rel_type in relation_results.keys():
            relation_results[rel_type][RECALL] = (
                round(
                    relation_results[rel_type][TP] / relation_results[rel_type][NB_GROUND_TRUTH], 2
                )
                if relation_results[rel_type][NB_GROUND_TRUTH] != 0
                else 0
            )
            relation_results[rel_type][PRECISION] = (
                round(relation_results[rel_type][TP] / relation_results[rel_type][NB_PRED], 2)
                if relation_results[rel_type][NB_PRED] != 0
                else 0
            )
            recall = relation_results[rel_type][RECALL]
            precision = relation_results[rel_type][PRECISION]
            relation_results[rel_type][F1_SCORE] = (
        ...
        macro_average = {
            RECALL: round(
                sum([rel_res[RECALL] for rel_type, rel_res in relation_results.items()])
            ...
            PRECISION: round(
                sum([rel_res[PRECISION] for rel_type, rel_res in relation_results.items()])
                / len(relation_results),
                2,
            )
            if len(relation_results) != 0
            ...
            F1_SCORE: round(
                sum([rel_res[F1_SCORE] for rel_type, rel_res in relation_results.items()])
                / len(relation_results),
                2,
            )
            if len(relation_results) != 0
            ...
            NB_GROUND_TRUTH: sum(
                [rel_res[NB_GROUND_TRUTH] for rel_type, rel_res in relation_results.items()]
            ),
        }
    ...
    def _load_entities(self, task_type: str) -> Tuple[List[str], List[str]]:
        ...
        if task_type == TASK_CONCEPT:
            ...
            filenames = [
                filename for filename in os.listdir(annotation_dir) if filename.endswith(".con")
            ]
        elif task_type == TASK_ASSERTION:
            annotation_dir = self.assertion_annotation_dir
            prediction_dir = self.assertion_prediction_dir
            filenames = [
                filename for filename in os.listdir(annotation_dir) if filename.endswith(".ast")
            ]
        else:
            raise NotImplementedError(f"Parsing of task {task_type} not implemented")

        for filename in filenames:
            ground_truth_entities = self._load_entity_annotation_file(
                os.path.join(annotation_dir, filename), task_type
            )
            prediction_entities = self._load_entity_annotation_file(
                os.path.join(prediction_dir, filename), task_type
            )
        ...
        flattened_ground_truth = [token_label for true in ground_truth for token_label in true]

        flattened_predictions = [token_label for pred in predictions for token_label in pred]
    ...
    def _load_input_text_file(path: str) -> List[List[Token]]:
        ...
        with open(path, "r", encoding="utf-8") as input_file:
            for i, line in enumerate(input_file.readlines()):
                tokens.append(
                    [
                        Token(label=O_TOKEN, text=word, line=i, word_index=index)
    ...
    @staticmethod
    def _build_tokens_annotated(
        entry_tokens: List[List[Token]], annotated_entities: List[EntityAnnotation]
    ) -> List[str]:
            ...
            if entity.start_line == entity.end_line:
                ...
                for index_line in range(entity.start_line, entity.end_line + 1):
                    if index_line == entity.start_line:
                        for index in range(
                            entity.start_word, len(copy_entries_tokens[index_line - 1]) - 1
                        ):
                        ...
                        for index in range(len(copy_entries_tokens[index_line - 1]) - 1):
                            copy_entries_tokens[index_line - 1][index].label = entity.label
        return [token.label for line in copy_entries_tokens for token in line]
    ...
    @staticmethod
    def _is_entity_equal(
        first_entity: EntityAnnotationForRelation, snd_entity: EntityAnnotationForRelation
    ) -> bool:
        return (
            (first_entity.start_line == snd_entity.start_line)
            and (first_entity.end_line == snd_entity.end_line)
            and (first_entity.start_word == snd_entity.start_word)
            and (first_entity.end_word == snd_entity.end_word)
        )
